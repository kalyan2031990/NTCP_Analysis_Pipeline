...
class ClinicalFactorsAnalyzer:
    ...
    def __init__(self, input_file, enhanced_output_dir):
    ...
    def load_and_merge_data(self):
        ...
        try:
            ...
            if 'PatientID' in self.ntcp_results.columns:
                unique_patients = self.ntcp_results['PatientID'].nunique()
                print(f" Unique patients: {unique_patients}")
            else:
                print("Warning: 'PatientID' column missing in NTCP results; unique patient count unavailable.")
        ...
        try:
            if not {'PatientID','Organ'}.issubset(set(self.clinical_data.columns)) or \
               not {'PatientID','Organ'}.issubset(set(self.ntcp_results.columns)):
                print("Error: Error: Both datasets must contain 'PatientID' and 'Organ' for merging.")
                return False
...
def main():
    ...
    try:
        # Initialize analyzer
        analyzer = ClinicalFactorsAnalyzer(input_file, enhanced_dir)
