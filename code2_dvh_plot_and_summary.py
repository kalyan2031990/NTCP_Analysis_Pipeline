...
def process(code1_dir: Path, outdir: Path):
    ...
    for _, r in master.iterrows():
        patient = str(r.Patient_AnoID)
        organ = r.Organ
        stem = f"{patient}_{organ}"
        ...
        # plots
        plot_cdvh(D, Vr, stem, metrics, cplot_dir)
        plot_ddvh(D, Vr, stem, metrics, organ in SERIAL_ORGANS, dplot_dir)

        overlay_buf.setdefault(patient, []).append({"Dose": D, "Vr": Vr, "Organ": organ, "Metrics": metrics})
        rows.append({"PatientID": patient, "Organ": organ, **metrics})

    # overlay plots
    for patient, entries in overlay_buf.items():
        plot_overlay(patient, entries, oplot_dir)
